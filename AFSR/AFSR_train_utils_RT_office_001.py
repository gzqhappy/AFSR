...
def get_classification_model_and_optimizer_twins(config):
    ...
    if config["train_with_GD"]:
        ...
        gd = robust_network.AdversarialNetwork_gradient(in_feature=in_feature, hidden_size=1024).cuda()
    ...
    for iter_num in range(config["num_iterations"]):
        ...
        try:
            ...
            if pgd20_acc > best_acc:
                ...
                state_dict = basic_net_sd.module.state_dict()
                torch.save(state_dict, os.path.join(config["output_path"], "best_acc_model_sd.pth.tar"))

                state_dict = criterion_cent_shift.state_dict()
                torch.save(state_dict, os.path.join(config["output_path"], "best_acc_model_scr.pth.tar"))
        ...
        if 'AFSR' in config["at"]:
            ...
            fea_org, logits_pred_out_org, _ = basic_net_td(inputs_target, multi_outputs=True)
        ...
        if 'AFSR' in config["at"]:
            ...
            fea_org_src, logits_pred_out_org_src, _ = basic_net_sd(inputs_source, multi_outputs=True)
        ...
        if config["train_with_GD"]:
            ...
            transfer_loss = cdan_loss.DANN(gradient, gd)
            adv_loss_sum += transfer_loss
