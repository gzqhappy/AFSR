...
def cwloss(output, target, confidence=50, num_classes=10):
    ...
    # loss = -torch.clamp(real - other + confidence, min=-1)  # equiv to max(..., 0.)
    loss = torch.sum(loss)
    return loss
...
def pgd(model, data, target, epsilon, step_size, num_steps, loss_fn, category, rand_init):
    ...
    for k in range(num_steps):
        ...
        with torch.enable_grad():
            ...
            if loss_fn == "cw":
                loss_adv = cwloss(output, target, num_classes=output.shape[-1])
        loss_adv.backward()
...
def pgd_reference_model(model, data, target, epsilon, step_size, num_steps, loss_fn, category, rand_init):
    model.eval()
    if category == "trades":
        x_adv = data.detach() + 0.001 * torch.randn(data.shape).cuda().detach() if rand_init else data.detach()
    if category == "Madry":
