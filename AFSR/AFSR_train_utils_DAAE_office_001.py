from MDD.model.MDD import MDD
from CDAN import network as cdan_network
from CDAN import loss as cdan_loss
...
def get_DANN(config):
    ...
    ad_net = cdan_network.AdversarialNetwork(base_network.output_num(), 1024)
    config["other_parameters"] = ad_net.get_parameters()
    config["ad_net"] = torch.nn.DataParallel(ad_net).cuda()
...
def get_classification_model_and_optimizer(config):
    basic_net = robust_network.RobustBasicNet(base_net='ResNet50Fc_Robust',
                                              resume_path=config["resume_path"],
                                              use_bottleneck=True,
                                              bottleneck_dim=config["bottleneck_dim"], width=config["width"],
                                              class_num=config["class_num"])
    basic_net.classifier_layer = config["classifier"]
    param_group = []

    if config["train_with_GD"]:
        in_feature = config["bottleneck_dim"]
        gd = robust_network.AdversarialNetwork_gradient(in_feature=in_feature, hidden_size=1024, coeff_times=0.5).cuda()
        param_group.extend(gd.get_parameters())
        config["gd"] = gd

        gd_ae = robust_network.AdversarialNetwork_gradient(in_feature=in_feature, hidden_size=1024,
                                                           coeff_times=0.5).cuda()
        param_group.extend(gd_ae.get_parameters())
        config["gd_ae"] = gd_ae

    ##########################################################
    if config["paradigm"] == 'uda':
        param_group.extend(basic_net.get_parameters(with_c=False))
        ...
        for k, v in basic_net.base_network.named_parameters():
            param_group += [{'params': v, 'lr': 1e-5}]
        for k, v in basic_net.bottleneck_layer.named_parameters():
            param_group += [{'params': v, 'lr': 1e-3}]
        optimizer = torch.optim.Adam(param_group, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-5)
        optimizer = op_copy(optimizer)
    ##########################################################

    basic_net = torch.nn.DataParallel(basic_net).cuda()
    config["basic_net"] = basic_net
...
def train(config):
    ...
    for iter_num in range(config["num_iterations"]):
        ...
        try:
            ...
            eval_ae = False
            if epoch <= config["decay_epoch_1"]:
                if epoch % 5 == 0:
                    eval_ae = True
            elif epoch <= config["decay_epoch_2"]:
                if epoch % 2 == 0:
                    eval_ae = True
            else:
                eval_ae = True
            ...
            if eval_ae:
                ...
                _, pgd20_acc = attack.eval_robust(basic_net, config["t_test_loader_eval_ae"], perturb_steps=20,
                                                  epsilon=0.031,
            ...
            # eval nat
            if epoch > 0:
                train_acc, _, _, _, _ = eval_with_nat(config["t_train_loader_eval"], basic_net, flag=False)
                test_acc, _, _, _, _ = eval_with_nat(config["t_test_loader_eval"], basic_net, flag=False)
            ...
            if pgd20_acc > best_acc:
                best_acc = pgd20_acc
                state_dict = basic_net.module.state_dict()
        ...
        if inputs_target.size()[0] == 1:
            ...
            continue
            # inputs_target, labels_target, tar_idx = iter_test.next()
        ...
        aux_net = pickle.loads(pickle.dumps(basic_net))
        aux_net.eval()
        basic_net.train()
        ...
        if 'AFSR' in config["at"]:
            x_2, x_org, _ = \
                get_ae_AFSR(aux_net, criterion_cent_shift, inputs_target, pseudo_labels)
            loss_cent, fea_2, logits_pred_out_2, fea_org, logits_pred_out_org = \
                AFSR_forward(basic_net, criterion_cent_shift, pseudo_labels, x_2, x_org)
        else:
            fea_org, logits_pred_out_org, softmax_outputs = basic_net(inputs_target, multi_outputs=True)

        # ###################### PGD
        x_adv = get_ae_pgd7(aux_net, inputs_target, pseudo_labels)
        fea, logits_pred, _ = basic_net(x_adv, multi_outputs=True)
        ...
        if 'AFSR' in config["at"]:
            x_2_src, x_org_src, _ = \
                get_ae_AFSR(aux_net, criterion_cent_shift, inputs_source, labels_source)
            loss_cent_src, fea_2_src, logits_pred_out_2_src, fea_org_src, logits_pred_out_org_src = \
                AFSR_forward(basic_net, criterion_cent_shift, labels_source, x_2_src, x_org_src)
        else:
            fea_org_src, logits_pred_out_org_src, _ = basic_net(inputs_source, multi_outputs=True)
        ###################### PGD
        x_adv_src = get_ae_pgd7(aux_net, inputs_source, labels_source)
        fea_src, logits_pred_src, _ = basic_net(x_adv_src, multi_outputs=True)
        ...
        if config["train_with_GD"]:
            ...
            loss_src_org = nn.CrossEntropyLoss(reduction='mean')(logits_pred_out_org_src, labels_source)
            loss_tgt_org = nn.CrossEntropyLoss(reduction='mean')(logits_pred_out_org, pseudo_labels)
            gradient_src_org = \
                torch.autograd.grad(outputs=loss_src_org, inputs=fea_org_src, create_graph=True, retain_graph=True,
                                    only_inputs=True)[0]
            gradient_tgt_org = \
                torch.autograd.grad(outputs=loss_tgt_org, inputs=fea_org, create_graph=True, retain_graph=True,
                                    only_inputs=True)[0]
            gradient = torch.cat((gradient_src_org, gradient_tgt_org), dim=0)
            transfer_loss_nat = cdan_loss.DANN(gradient, gd)
            adv_loss_sum += transfer_loss_nat

            loss_tgt_adv = nn.CrossEntropyLoss(reduction='mean')(logits_pred, pseudo_labels)
            gradient_tgt_adv = \
                torch.autograd.grad(outputs=loss_tgt_adv, inputs=fea, create_graph=True, retain_graph=True,
                                    only_inputs=True)[0]
            gradient = torch.cat((gradient_src_org, gradient_tgt_adv), dim=0)
            transfer_loss_ae = cdan_loss.DANN(gradient, gd_ae)
            adv_loss_sum += transfer_loss_ae
        ...
        at_loss = adv_loss_src / bs_src

        loss_center_AFSR = ((loss_cent_src / bs_src) * config["AFSR_coefficient"]) / 2
        adv_loss_sum += (at_loss + loss_center_AFSR)
